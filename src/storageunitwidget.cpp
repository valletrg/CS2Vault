...
StorageUnitWidget::StorageUnitWidget(SteamCompanion *companion, QWidget *parent)
    : QWidget(parent)
    , companion(companion)
{
    setupUI();

    // When GC is ready, automatically load the inventory
    ...
    connect(companion, &SteamCompanion::inventoryReceived,
            this, [this](const QList<GCItem> &items, const QList<GCContainer> &newContainers) {
        if (!items.isEmpty()) {
        ...
        if (!items.isEmpty()) {
            setStatus(QString("Inventory loaded — %1 items. Select a storage unit above.")
                      .arg(items.size()), "#38C878");
        }
        setBusy(false);
    });

    connect(companion, &SteamCompanion::storageUnitReceived,
            this, [this](const QString &casketId, const QList<GCItem> &items) {
        currentCasketId = casketId;
        storageItems = items;
        populateStorageTable(items);
        setStatus(QString("Storage unit loaded — %1 item(s).")
                  .arg(items.size()), "#38C878");
        setBusy(false);
    });

    connect(companion, &SteamCompanion::transferComplete,
            this, &StorageUnitWidget::onTransferComplete);

    connect(companion, &SteamCompanion::errorOccurred,
...
void StorageUnitWidget::setupUI()
{
    ...
    statusLabel = new QLabel("Sign in to load your inventory and storage units.", this);
    ...
    storageTable->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Stretch);
    storageTable->horizontalHeader()->setSectionResizeMode(1, QHeaderView::ResizeToContents);
    storageTable->horizontalHeader()->setSectionResizeMode(2, QHeaderView::ResizeToContents);
    ...
    moveToInventoryButton = new QPushButton("→  Move Selected to Inventory", leftWidget);
    ...
    inventoryTable->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Stretch);
    inventoryTable->horizontalHeader()->setSectionResizeMode(1, QHeaderView::ResizeToContents);
    inventoryTable->horizontalHeader()->setSectionResizeMode(2, QHeaderView::ResizeToContents);
    ...
    moveToStorageButton = new QPushButton("←  Move Selected to Storage Unit", rightWidget);
    ...
    connect(refreshButton,         &QPushButton::clicked, this, &StorageUnitWidget::onRefreshClicked);
    connect(moveToInventoryButton, &QPushButton::clicked, this, &StorageUnitWidget::onMoveToInventory);
    connect(moveToStorageButton,   &QPushButton::clicked, this, &StorageUnitWidget::onMoveToStorageUnit);

    connect(storageSelectAllButton,  &QPushButton::clicked, storageTable,   &QTableWidget::selectAll);
    connect(storageSelectNoneButton, &QPushButton::clicked, storageTable,   &QTableWidget::clearSelection);
    connect(inventorySelectAllButton,  &QPushButton::clicked, inventoryTable, &QTableWidget::selectAll);
    connect(inventorySelectNoneButton, &QPushButton::clicked, inventoryTable, &QTableWidget::clearSelection);

    connect(storageTable->selectionModel(), &QItemSelectionModel::selectionChanged,
            this, [this]() {
        moveToInventoryButton->setEnabled(
            !storageTable->selectedItems().isEmpty() && !currentCasketId.isEmpty());
    });
    connect(inventoryTable->selectionModel(), &QItemSelectionModel::selectionChanged,
            this, [this]() {
        moveToStorageButton->setEnabled(
            !inventoryTable->selectedItems().isEmpty() && !currentCasketId.isEmpty());
    });

    connect(inventorySearchEdit, &QLineEdit::textChanged, this, [this](const QString &text) {
...
void StorageUnitWidget::onContainersUpdated(const QList<GCContainer> &newContainers)
{
...
void StorageUnitWidget::onStorageUnitSelected(int index)
{
    ...
    currentCasketId = containers[index].id;
    storageTable->setRowCount(0);
    setStatus(QString("Loading %1...").arg(containers[index].name), "#5A9BE6");
    setBusy(true);
...
void StorageUnitWidget::onMoveToInventory()
{
    ...
    if (selected.isEmpty() || currentCasketId.isEmpty()) return;
    ...
    if (QMessageBox::question(this, "Confirm Move",
        QString("Move %1 item(s) from storage unit to your inventory?").arg(count),
...
void StorageUnitWidget::onMoveToStorageUnit()
{
    ...
    if (QMessageBox::question(this, "Confirm Move",
        QString("Move %1 item(s) to storage unit '%2'?")
            .arg(count).arg(storageCombo->currentText()),
        QMessageBox::Yes | QMessageBox::No) != QMessageBox::Yes) return;

    setBusy(true);
    setStatus(QString("Moving %1 item(s) to storage unit...").arg(count), "#F0A500");
                                            ...
                                            const QString &itemId)
{
    ...
    setStatus(action == "added"
        ? "Item moved to storage unit. Refreshing..."
        : "Item moved to inventory. Refreshing...", "#38C878");
...
void StorageUnitWidget::populateStorageTable(const QList<GCItem> &items)
{
    ...
    for (int i = 0; i < items.size(); ++i) {
        ...
        storageTable->setItem(i, 1, new QTableWidgetItem(
            item.exterior.isEmpty() ? "—" : item.exterior));
        storageTable->setItem(i, 2, new QTableWidgetItem(
            item.paintWear > 0 ? QString::number(item.paintWear, 'f', 6) : "—"));
    }
    storageTable->setSortingEnabled(true);
    moveToInventoryButton->setEnabled(false);
}
...
void StorageUnitWidget::populateInventoryTable(const QList<GCItem> &items)
{
    ...
    for (int i = 0; i < items.size(); ++i) {
        ...
        inventoryTable->setItem(i, 1, new QTableWidgetItem(
            item.exterior.isEmpty() ? "—" : item.exterior));
        inventoryTable->setItem(i, 2, new QTableWidgetItem(
            item.paintWear > 0 ? QString::number(item.paintWear, 'f', 6) : "—"));
    }
    inventoryTable->setSortingEnabled(true);
    moveToStorageButton->setEnabled(false);
    ...
    if (!search.isEmpty()) {
        for (int i = 0; i < inventoryTable->rowCount(); ++i) {
            ...
            inventoryTable->setRowHidden(i,
                it && !it->text().contains(search, Qt::CaseInsensitive));
        }
    }
}
...
void StorageUnitWidget::setBusy(bool busy)
{
    ...
    moveToInventoryButton->setEnabled(!busy && !storageTable->selectedItems().isEmpty());
    moveToStorageButton->setEnabled(!busy && !inventoryTable->selectedItems().isEmpty());
}