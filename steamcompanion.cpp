    ...
    , process(new QProcess(this))
{
    connect(process, &QProcess::readyReadStandardOutput,
            this, &SteamCompanion::onReadyRead);
    connect(process, &QProcess::errorOccurred,
            this, &SteamCompanion::onProcessError);
    connect(process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
            this, &SteamCompanion::onProcessFinished);
}
...
bool SteamCompanion::isGCReady() const
{
    return gcReady_;
}
...
void SteamCompanion::handleMessage(const QJsonObject &msg)
{
    ...
    if (type == "status") {
        ...
        if (state == "qr_code") {
            ...
            // Companion sends container list once GC inventory is loaded
            ...
            if (!containers.isEmpty()) {
                // Re-emit as part of a statusMessage so the UI can show them
                QString info = QString("GC inventory loaded. Storage units: ");
                QStringList names;
                for (const GCContainer &c : containers)
                    names << QString("%1 (%2)").arg(c.name).arg(c.id);
                emit statusMessage(info + names.join(", "));
            }
...
void SteamCompanion::onProcessError(QProcess::ProcessError error)
{
    ...
    switch (error) {
        case QProcess::FailedToStart:
            msg = "Failed to start companion process. Check that node is at /usr/bin/node.";
            break;
...
void SteamCompanion::onProcessFinished(int exitCode, QProcess::ExitStatus status)
{
    gcReady_ = false;
    if (status == QProcess::CrashExit) {
        emit errorOccurred("Companion process exited unexpectedly.");
    } else if (exitCode != 0) {
        emit statusMessage(QString("Companion process exited with code %1.").arg(exitCode));
    }
}
