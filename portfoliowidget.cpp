    ...
    , loadingDialog(nullptr)
{
    ...
    connect(steamApi, &SteamAPI::inventoryFetched, this, &PortfolioWidget::onSteamInventoryFetched);
    connect(steamApi, &SteamAPI::inventoryError, this, [this](const QString &error) {
        if (loadingDialog) {
            loadingDialog->close();
    ...
    connect(portfolioManager, &PortfolioManager::portfolioChanged, this, [this](const QString &id) {
    ...
    connect(autoSaveTimer, &QTimer::timeout, portfolioManager, &PortfolioManager::saveToFile);
    autoSaveTimer->start(30000);
    priceCheckTimer = new QTimer(this);
    priceCheckTimer->setInterval(2000); // one request per 2 seconds
    connect(priceCheckTimer, &QTimer::timeout, this, &PortfolioWidget::onPriceCheckTick);
}
...
void PortfolioWidget::setupUI()
{
    ...
    portfolioTable->setHorizontalHeaderLabels({
        "Skin Name", "Condition", "Float", "Qty", 
        "Buy Price", "Current", "Profit/Loss", "ROI %"
    });
    ...
    QGroupBox *statsGroup = new QGroupBox("Statistics", bottomWidget);
    QFormLayout *statsLayout = new QFormLayout(statsGroup);
    
    totalInvestmentLabel = new QLabel("$0.00", statsGroup);
    currentValueLabel = new QLabel("$0.00", statsGroup);
    totalProfitLabel = new QLabel("$0.00", statsGroup);
    roiLabel = new QLabel("0.00%", statsGroup);
    
    QFont bold;
    bold.setBold(true);
    totalInvestmentLabel->setFont(bold);
    currentValueLabel->setFont(bold);
    totalProfitLabel->setFont(bold);
    roiLabel->setFont(bold);
    
    statsLayout->addRow("Total Investment:", totalInvestmentLabel);
    statsLayout->addRow("Current Value:",    currentValueLabel);
    statsLayout->addRow("Profit/Loss:",      totalProfitLabel);
    statsLayout->addRow("ROI:",              roiLabel);
    
    statsAndSteam->addWidget(statsGroup, 1);
    
    // Steam import -- collapsed behind a button to save space
    // Keep the actual widgets alive (needed by slots) but put them in a dialog
    steamIdEdit       = new QLineEdit(this);
    steamIdEdit->setPlaceholderText("Steam ID (76561198...)");
    steamLoginButton  = new QPushButton("Connect", this);
    importSteamButton = new QPushButton("Import Inventory", this);
    importSteamButton->setEnabled(false);
    steamStatusLabel  = new QLabel("Not connected", this);
    // Hide the raw widgets -- they'll be re-parented into the dialog on demand
    steamIdEdit->hide();
    steamLoginButton->hide();
    importSteamButton->hide();
    steamStatusLabel->hide();
    
    steamImportButton = new QPushButton("Steam Import…", bottomWidget);
    steamImportButton->setFixedHeight(32);
    steamImportButton->setToolTip("Import your CS2 inventory from Steam");
    QVBoxLayout *steamBtnLayout = new QVBoxLayout();
    steamBtnLayout->addStretch();
    steamBtnLayout->addWidget(steamImportButton);
    steamBtnLayout->addStretch();
    statsAndSteam->addLayout(steamBtnLayout);
    
    bottomLayout->addLayout(statsAndSteam);
    
    // Wire up the Steam import button to open a compact dialog
    connect(steamImportButton, &QPushButton::clicked, this, [this]() {
        QDialog *dlg = new QDialog(this);
        dlg->setWindowTitle("Steam Import");
        dlg->setMinimumWidth(380);
        
        QFormLayout *form = new QFormLayout(dlg);
        
        QLineEdit *idEdit = new QLineEdit(steamIdEdit->text(), dlg);
        idEdit->setPlaceholderText("Steam ID (76561198...)");
        
        QPushButton *connectBtn  = new QPushButton("Connect",          dlg);
        QPushButton *importBtn   = new QPushButton("Import Inventory",  dlg);
        QLabel      *statusLbl   = new QLabel(steamStatusLabel->text(), dlg);
        statusLbl->setStyleSheet(steamStatusLabel->styleSheet());
        importBtn->setEnabled(steamApi->isLoggedIn());
        
        QHBoxLayout *btnRow = new QHBoxLayout();
        btnRow->addWidget(connectBtn);
        btnRow->addWidget(importBtn);
        
        form->addRow("Steam ID:", idEdit);
        form->addRow("",         btnRow);
        form->addRow("Status:",  statusLbl);
        
        connect(connectBtn, &QPushButton::clicked, this, [this, idEdit, statusLbl, importBtn]() {
            steamIdEdit->setText(idEdit->text());
            statusLbl->setText("Connecting...");
            statusLbl->setStyleSheet("color: #5A9BE6;");
            steamApi->loginWithSteamId(idEdit->text().trimmed());
            // Mirror status back when slot fires
            QTimer::singleShot(100, this, [this, statusLbl, importBtn]() {
                statusLbl->setText(steamStatusLabel->text());
                statusLbl->setStyleSheet(steamStatusLabel->styleSheet());
                importBtn->setEnabled(steamApi->isLoggedIn());
            });
        });
        ...
        connect(importBtn, &QPushButton::clicked, this, [this, dlg]() {
            if (!steamApi->isLoggedIn()) return;
            SteamProfile profile = steamApi->getProfile();
            steamStatusLabel->setText("Fetching inventory...");
            steamStatusLabel->setStyleSheet("color: #5A9BE6;");
            loadingDialog = new QProgressDialog("Loading Steam inventory...", "Cancel", 0, 0, this);
            loadingDialog->setWindowModality(Qt::WindowModal);
            loadingDialog->setCancelButton(nullptr);
            loadingDialog->show();
            steamApi->fetchInventory(profile.steamId);
            dlg->accept();
        });
    ...
    // Sync status label back after login slots fire
    connect(steamApi, &SteamAPI::loginSuccessful, this, [this]() {
        steamStatusLabel->setText(QString("Connected: %1").arg(steamApi->getProfile().personaName));
        steamStatusLabel->setStyleSheet("color: #38C878;");
        importSteamButton->setEnabled(true);
    });
    connect(steamApi, &SteamAPI::loginFailed, this, [this](const QString &err) {
        steamStatusLabel->setText(QString("Failed: %1").arg(err));
        steamStatusLabel->setStyleSheet("color: #DC4646;");
    });
...
connect(queuePauseButton, &QPushButton::clicked, this, [this]() {
    ...
    connect(portfolioComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &PortfolioWidget::onPortfolioSelected);
    connect(createPortfolioButton, &QPushButton::clicked, this, &PortfolioWidget::onCreatePortfolio);
    connect(deletePortfolioButton, &QPushButton::clicked, this, &PortfolioWidget::onDeletePortfolio);
    connect(renamePortfolioButton, &QPushButton::clicked, this, &PortfolioWidget::onRenamePortfolio);
    
    connect(addButton, &QPushButton::clicked, this, &PortfolioWidget::onAddItem);
    connect(removeButton, &QPushButton::clicked, this, &PortfolioWidget::onRemoveItem);
    connect(editButton, &QPushButton::clicked, this, &PortfolioWidget::onEditItem);
    connect(exportButton, &QPushButton::clicked, this, &PortfolioWidget::onExportCSV);
    connect(importButton, &QPushButton::clicked, this, &PortfolioWidget::onImportCSV);
    connect(refreshButton, &QPushButton::clicked, this, &PortfolioWidget::onRefreshPrices);
    
    // steamLoginButton and importSteamButton are handled via the Steam Import dialog now;
    // no direct connects needed here for them.
...
void PortfolioWidget::refreshPortfolioList()
{
    ...
    if (!portfolios.isEmpty()) {
        ...
        loadPortfolioItems();
        // Seed the first history dot if the portfolio already has items but no history yet
...
void PortfolioWidget::populateTable()
{
    ...
    for (int i = 0; i < portfolio.items.size(); ++i) {
        ...
        double roi = item.buyPrice > 0 ? ((item.currentPrice - item.buyPrice) / item.buyPrice) * 100.0 : 0.0;
        ...
        QTableWidgetItem *floatItem = new QTableWidgetItem(QString::number(item.floatValue, 'f', 6));

        NumericTableWidgetItem *qtyItem = new NumericTableWidgetItem(QString::number(item.quantity));
        qtyItem->setData(Qt::UserRole, item.quantity);

        NumericTableWidgetItem *buyItem = new NumericTableWidgetItem(QString("$%1").arg(item.buyPrice, 0, 'f', 2));
        buyItem->setData(Qt::UserRole, item.buyPrice);

        NumericTableWidgetItem *currentItem = new NumericTableWidgetItem(QString("$%1").arg(item.currentPrice, 0, 'f', 2));
        currentItem->setData(Qt::UserRole, item.currentPrice);

        NumericTableWidgetItem *profitItem = new NumericTableWidgetItem(QString("$%1").arg(profit, 0, 'f', 2));
        profitItem->setData(Qt::UserRole, profit);
        profitItem->setForeground(profit >= 0 ? QColor(0, 150, 0) : QColor(200, 0, 0));

        NumericTableWidgetItem *roiItem = new NumericTableWidgetItem(QString("%1%").arg(roi, 0, 'f', 2));
...
void PortfolioWidget::calculateStatistics()
{
    ...
    totalProfitLabel->setStyleSheet(profit >= 0 ? "color: green;" : "color: red;");
...
void PortfolioWidget::updateChart()
{
    ...
    for (const PortfolioHistoryPoint &pt : portfolio.history) {
        ...
        if (pt.totalValue > maxVal)  maxVal = pt.totalValue;
        if (pt.totalCost  > maxVal)  maxVal = pt.totalCost;
        if (pt.totalValue < minVal)  minVal = pt.totalValue;
        if (pt.totalCost  < minVal)  minVal = pt.totalCost;
        if (pt.timestamp * 1000 > maxTime) maxTime = pt.timestamp * 1000;
        if (pt.timestamp * 1000 < minTime) minTime = pt.timestamp * 1000;
    }
    ...
    if (currentCost  > maxVal) maxVal = currentCost;
    if (currentValue < minVal) minVal = currentValue;
    if (currentCost  < minVal) minVal = currentCost;
    if (nowMs > maxTime) maxTime = nowMs;
    if (nowMs < minTime) minTime = nowMs;
    
    if (minVal == 1e12) minVal = 0;
    
    double padding = (maxVal - minVal) * 0.12;
    if (padding < 5.0) padding = 5.0;
...
{
    QString name = QInputDialog::getText(this, "Create Portfolio", "Portfolio name:");
...
{
    if (currentPortfolioId.isEmpty()) return;
...
void PortfolioWidget::onRenamePortfolio()
{
    ...
    QString newName = QInputDialog::getText(this, "Rename Portfolio", 
                                            "New name:", QLineEdit::Normal, portfolio.name);
...
void PortfolioWidget::onAddItem()
{
    ...
    QStringList weapons = {
        ...
        "★ Falchion Knife", "★ Shadow Daggers", "★ Gut Knife",
        "★ Flip Knife", "★ Navaja Knife", "★ Stiletto Knife",
        "★ Talon Knife", "★ Ursus Knife", "★ Huntsman Knife",
        "★ Bowie Knife", "★ Skeleton Knife", "★ Kukri Knife",
        // Gloves
    ...
    conditionCombo.addItems({"Factory New", "Minimal Wear", "Field-Tested", "Well-Worn", "Battle-Scarred"});
    ...
    connect(&weaponCombo, &QComboBox::currentTextChanged, [&](const QString&) { updatePreview(); });
    connect(&skinNameEdit, &QLineEdit::textChanged, [&](const QString&) { updatePreview(); });
...
void PortfolioWidget::onEditItem()
{
    ...
    Portfolio portfolio = portfolioManager->getPortfolio(currentPortfolioId);
    if (row >= portfolio.items.size()) return;
...
void PortfolioWidget::onSteamInventoryFetched(const QVector<SteamInventoryItem> &items)
{
    if (loadingDialog) {
        loadingDialog->close();
        loadingDialog->deleteLater();
...
void PortfolioWidget::showImportDialog(const QVector<SteamInventoryItem> &items)
{
    QDialog dialog(this);
    ...
    QLabel *infoLabel = new QLabel(QString("Found %1 items. Select items to import:").arg(items.size()));
    ...
    if (match.hasMatch()) {
        ...
        portfolioItem.condition = "Unknown";
                ...
                portfolioItem.purchaseDate = QDate::currentDate().toString("yyyy-MM-dd");
...
void PortfolioWidget::onExportCSV()
{
    ...
    QString fileName = QFileDialog::getSaveFileName(this, "Export Portfolio", 
                                                    "portfolio.csv", "CSV Files (*.csv)");
    ...
    out << "Skin Name,Condition,Float,Quantity,Buy Price,Current Price,Purchase Date,Notes\n";
...
void PortfolioWidget::enqueuePriceCheck(const QString &portfolioId, int itemIndex, const QString &marketName)
{
    PriceCheckJob job;
